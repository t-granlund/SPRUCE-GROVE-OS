...
class TestLoadCatalogSkillIds:
    ...
    def test_success(self):
        from spruce_grove.command_line.skills_completion import load_catalog_skill_ids
...
class TestSkillsCompleterGetCompletions:
    ...
    def setup_method(self):
        from spruce_grove.command_line.skills_completion import SkillsCompleter
...
class TestLoadContextCompletionException:
    ...
    def test_exception_in_glob_silently_ignored(self):
        ...
        with patch(
            "spruce_grove.command_line.load_context_completion.CONFIG_DIR",
            "/nonexistent/x/y/z",
        ):
...
class TestModelSwitching:
    ...
    def test_get_effective_agent_model_success(self):
        from spruce_grove.model_switching import _get_effective_agent_model
    ...
    def test_refresh_context_status_uses_effective_model_capacity(self):
        from spruce_grove.model_switching import _refresh_context_status
        ...
        with patch(
            "spruce_grove.messaging.spinner.update_spinner_context"
        ) as update_status:
    ...
    def _run(self, model_name, agent=None):
        ...
        from spruce_grove.model_switching import set_model_and_reload_agent
        ...
        with patch("spruce_grove.model_switching.set_model_name"):
            with patch("spruce_grove.messaging.emit_warning", fake_warn):
                with patch("spruce_grove.messaging.emit_info", fake_info):
                    with patch(
                        "spruce_grove.agents.get_current_agent", return_value=agent
                    ):
    ...
    def test_reload_refreshes_context_status(self):
        agent = MagicMock()
        agent.get_model_name.return_value = "model-x"
        with patch("spruce_grove.model_switching._refresh_context_status") as refresh:
            self._run("model-x", agent=agent)
...
class TestMarkdownPatches:
    ...
    def test_left_justified_heading_h1(self):
        ...
        from spruce_grove.messaging.markdown_patches import LeftJustifiedHeading
    ...
    def test_patch_idempotent(self):
        ...
        from spruce_grove.messaging import markdown_patches
...
class TestErrorLoggingRotation:
    ...
    def test_rotate_log_when_too_large(self):
        from spruce_grove.error_logging import MAX_LOG_SIZE, _rotate_log_if_needed
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("spruce_grove.error_logging.ERROR_LOG_FILE", log_file):
    ...
    def test_rotate_log_oserror_caught(self):
        ...
        from spruce_grove.error_logging import _rotate_log_if_needed

        with patch("spruce_grove.error_logging.os.path.exists", return_value=True):
            with patch(
                "spruce_grove.error_logging.os.path.getsize",
