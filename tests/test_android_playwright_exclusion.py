...
def test_browser_tool_registry_skips_imports_on_android():
    from spruce_grove.tools import _load_browser_tool_registry

    with (
        patch("spruce_grove.tools.sys.platform", "android"),
    ...
    assert registry == {}
    assert not any(
        module.startswith("spruce_grove.tools.browser") for module in imported_modules
    )


def test_playwright_agents_are_skipped_on_android():
    from spruce_grove.agents.agent_manager import _builtin_agent_modules_to_skip

    with patch("spruce_grove.agents.agent_manager.sys.platform", "android"):
