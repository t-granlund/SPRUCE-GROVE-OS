...
def test_sync_from_config_drops_servers_absent_from_json(tmp_path, monkeypatch):
    monkeypatch.setattr("spruce_grove.config.DATA_DIR", str(tmp_path))

    present = {
        "proj-stdio": {"type": "stdio", "command": "/usr/bin/true", "enabled": True}
    }

    with patch("spruce_grove.config.load_mcp_server_configs", return_value=present):
        manager = MCPManager()

    assert manager.registry.get_by_name("proj-stdio") is not None

    with patch("spruce_grove.config.load_mcp_server_configs", return_value={}):
...
def test_sync_from_config_keeps_registry_when_json_unreadable(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("spruce_grove.config.MCP_SERVERS_FILE", str(servers_file))
    monkeypatch.setattr(
        "spruce_grove.mcp_.project_config.load_project_mcp_server_configs",
        lambda *args, **kwargs: {},
    )
