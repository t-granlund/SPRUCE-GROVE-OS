...
def test_every_builtin_defaults_to_point_two_except_the_classic():
    ...
    grove is the lone exception: the kennel bounce trots at 0.06s.
    """
...
def test_paint_gap_separates_frame_from_status(monkeypatch):
    ...
    monkeypatch.setattr(
        "spruce_grove.messaging.bottom_bar.get_bottom_bar", lambda: FakeBar()
    )
    rc._paint_prefix("(x) ")
    rc._clear_prefix()
...
def test_default_spinner_matches_rc_frames():
    """DRY check: rc.FRAMES *is* the catalogue's grove entry."""
