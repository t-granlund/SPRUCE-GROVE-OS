...
def _ctx(agent_name: str = "spruce-grove") -> Any:
...
def test_resolve_wing_shortcuts(kennel_root: Path) -> None:
    ...
    assert tools._resolve_wing("", "spruce-grove", cwd).startswith("repo:")
    assert tools._resolve_wing("repo", "spruce-grove", cwd).startswith("repo:")
    assert tools._resolve_wing("agent", "spruce-grove", cwd) == "agent:spruce-grove"
    assert tools._resolve_wing("user", "spruce-grove", cwd) == "user:default"
    assert tools._resolve_wing("custom:name", "spruce-grove", cwd) == "custom:name"
...
def test_list_wings_with_counts(kennel_root: Path) -> None:
    ...
    assert "agent:spruce-grove" not in names
    for w in out.wings:
