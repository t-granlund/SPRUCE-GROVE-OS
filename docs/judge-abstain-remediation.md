...
| G2 | Harness runtime predates the fix, so keyed cycles still abstain | INSTALL DONE 2026-09-10 ~20:10Z; live re-run pending | user (launch + key) | Diagnosed 18:58Z: the /goal harness executed the installed uv-tool `code-puppy` 0.0.830 (legacy namespace, no `tolerant_openai.py`, legacy `~/.code_puppy` DATA_DIR with all judges disabled -> `[default]` fallback). RESOLUTION STEP COMPLETE: `uv tool install --from ~/SPRUCE-GROVE-OS spruce-grove` now provides `spruce-grove`/`grove` 0.1.0 on PATH with the tolerant wrapper verified present, DATA_DIR `~/.spruce_grove`, compat shim live, plugin bundle 0.0.48 resolved. Remaining to flip the gate: launch /goal cycles via `spruce-grove` (not `code-puppy`) with `SYNTHETIC_API_KEY` in the launch environment (key absent from this machine's shell env, keychain, legacy puppy.cfg, and rc files -- owner secret errand), then one keyed cycle must show a non-ABSTAIN verdict |
