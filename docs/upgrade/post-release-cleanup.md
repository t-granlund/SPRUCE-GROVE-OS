...
`spruce_grove.claude_cache_client.patch_anthropic_client_messages` — those
...
- `spruce_grove/callbacks.py` thin wrappers: `on_create_file` (:457),
  `on_replace_in_file` (:461), `on_delete_snippet` (:465),
  `on_post_autosave` (:485), `on_message` (:1344).
- `BaseAgent.append_to_message_history` — `spruce_grove/agents/base_agent.py:190`.
- Sync token-refresh twins in `spruce_grove/claude_cache_client.py`:
...
- `stream_event` callback timing (flagged by the plugins repo during the
  ...
  `spruce_grove/agents/event_stream_handler.py:_fire_stream_event`, so the
