...
>
> **Scope.** Three surfaces, two of them literally titled "Spruce Grove Field
...
|---|---|---|---|
| **Title** | "The Spruce Grove Field Guide · Live System Inventory · v4" | "Spruce Grove Field Guide" | "Spruce Grove, documented." |
| **Path** | `~/agent-venn.html` *(outside repo)* | `spruce_grove/docs/field-guide/` | `spruce_grove/pages-hub/` |
...
Hero: kicker `THE SPRUCE GROVE FIELD GUIDE · LIVE SYSTEM INVENTORY · v4`,
...
4. `#anatomy` — "Three minds, one decision loop" — 3-circle SVG Venn (Architect / web-grove / web-retriever / Shared) with tab + circle-click swap
...
- **`~/extract_field_guide.py`** — imports the live `spruce_grove` package
  ...
  `~/field_guide_data.json`. Run: `uv run --project ~/spruce_grove python ~/extract_field_guide.py`
...
## Surface B — Repo field guide (`spruce_grove/docs/field-guide/`)
...
- **`data.js`** (296 KB) — `window.FIELD_GUIDE_DATA = {…}`. Generated artifact;
  **do not hand-edit** (regenerated by `generate-field-guide.py`). Current
  snapshot: `generatedAt 2026-08-22`, `spruce-grove v0.0.768`, head `1fb151bc`,
...
- **`assets/`** — `spruce_grove_logo_noback.png` (nav logo), `favicon.png` (favicon), `grove.svg`/`grove-full.svg` (legacy SVG marks).

### Sections (12)

`hero`/stats → `overview` (What is Spruce Grove? + philosophy cards) →
...
### Generators (in `spruce_grove/docs/`)
...
- **`docs/generate-field-guide.py`** (24 KB) — reads the live repo (git log,
  ...
  `docs/field-guide-flat.html`. Run: `cd ~/spruce_grove && python docs/generate-field-guide.py`
...
## Surface C — pages-hub public mirror (`spruce_grove/pages-hub/`)

"Spruce Grove, documented." — the public knowledge-base mirror
(`mpfaffenberger/spruce_grove` → `t-granlund/spruce_grove`), rebuilt daily by an
...
`tokens.css` (design tokens), `spruce_grove_logo_noback.png` (brand logo), `favicon.png`, `grove.svg`/`grove-full.svg` (legacy SVG marks).
...
| `docs/field-guide/assets/*` | grove svgs |
...
1. **B — repo field guide data:**
   `cd ~/spruce_grove && python docs/generate-field-guide.py`
   → rewrites `docs/field-guide/data.js` + `docs/field-guide-flat.html`.
2. **C — release observatory (depends on B):**
   `cd ~/spruce_grove && uv run python pages-hub/generate-updates.py`
...
- The Leather Apron Club presentation lives in its canonical home: `~/code_puppy/docs/decks/the-great-adpuppytion/leather-apron-club/` (51 slides; established Sept 18, first council Sept 25).
