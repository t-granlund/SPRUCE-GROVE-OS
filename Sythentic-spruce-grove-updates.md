...
\* **\*\*Goal:\*\*** Re-pin Spruce Grove agents to eliminate premature quota exhaustion on the Synthetic.new 90x plan.  
...
| :--- | :--- | :--- | :---: | :--- |  
| \`spruce-grove\` | \`syn:large:vision\` | \`syn:large:text\` | **\*\*1.0×\*\*** | Main driving seat; shift reasoning off 3.0× Kimi-K3 |  
...
| \`web-grove\` | \`hf:zai-org/GLM-4.7-Flash\` | \`syn:small:text\` | **\*\*0.1×\*\*** | Scraping and DOM parsing |
...
**4\. Execution Workflow (Dolt / Beads DB \+ Spruce Grove CLI)**
...
bd create task "Optimize Synthetic.new agent model pins" \\  
  \--description "Re-pin spruce-grove, husky, and fan-out agents to reduce multiplier burn" \\  
...
\# Apply configuration changes via Spruce Grove CLI  
spruce-grove config apply \--file grove.cfg

\# Execute database migrations for telemetry tracking  
spruce-grove db migrate
...
bd commit TASK-001 \-m "Updated grove.cfg, agent JSON pins, and index.html"  
bd close TASK-001

\# Git commit and deploy to GitHub Pages  
git add grove.cfg agents/\*.json docs/index.html synthetic-90x-cockpit.html  
