...
``spruce_grove.agents._key_listeners.suspended_key_listener``: wrap any code
...
class BottomBar(TranscriptGuardMixin, BarPainterMixin):
    ...
    def set_status_prefix(self, text: str) -> None:
        ...
        Separate from :meth:`set_status` so an animation (the grove
    ...
    def _register_atexit(self) -> None:
        ...
        region because Spruce Grove died without a clean ``stop()``.
        """
        with self._lock:
...
def _use_inline_surface() -> bool:
    ...
    mode = os.environ.get("SPRUCE_GROVE_PROMPT_MODE", "auto").strip().lower()
