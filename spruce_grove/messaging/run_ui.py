...
* :func:`start_run_ui` — starts the bottom bar (scroll region), creates a
  :class:`~spruce_grove.messaging.line_editor.RunningLineEditor`, and
...
Import note: the key-listener lives in ``spruce_grove.agents`` which imports
``spruce_grove.messaging`` — so this module lazy-imports it inside functions
...
def stop_persistent_ui() -> None:
    ...
    if handle is not None:
        try:
            from spruce_grove.agents._key_listeners import set_active_handle
...
def _set_feed_target(editor: Optional[RunningLineEditor]) -> None:
    ...
    try:
        from spruce_grove.agents._key_listeners import set_line_editor
    except ImportError:
