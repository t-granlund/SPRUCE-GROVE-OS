"""DEPRECATED compat shim — the Rich Live grove spinner is gone.

Phase 3 of the bottom-bar rewrite replaced the spinner with a persistent
bottom prompt: a DECSTBM scroll region managed by
``spruce_grove.messaging.bottom_bar``, with the old spinner "context info"
...
New code should use ``spruce_grove.messaging.bottom_bar`` directly.
"""
...
def pause_all_spinners() -> None:
    ...
    ``spruce_grove.messaging.run_ui.suspended_run_ui()`` instead.
    """
...
def _codex_usage_suffix() -> str:
    ...
    try:
        from spruce_grove.callbacks import get_usage_status
