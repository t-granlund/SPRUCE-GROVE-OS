...
from spruce_grove.plugins import trust as _trust
...
_PROJECT_PLUGIN_PYCACHE = str(Path.home() / ".spruce_grove" / "plugin_bytecode_cache")
...
def _load_user_plugins(
    ...
    """Load user plugins from ~/.spruce_grove/plugins/.
...
def _load_project_plugins(
    ...
    """Load TRUSTED project plugins from <CWD>/.spruce_grove/plugins/.
    ...
    Returns list of successfully loaded plugin names.
    """
    from spruce_grove.plugins.config import is_plugin_disabled
...
def get_project_plugins_directory() -> Path | None:
    ...
    Looks for a .spruce_grove/plugins/ directory in the current working directory.
    ...
    """
    project_plugins_dir = Path.cwd() / ".spruce_grove" / "plugins"
    if not project_plugins_dir.is_dir():
        return None
...
def load_plugin_callbacks() -> dict[str, list[str]]:
    ...
    Loads plugins from:
    1. Installed ``spruce_grove.plugins`` entry points (builtin tier)
    2. Legacy bundled directories, when present during migration
    3. User plugins in ~/.spruce_grove/plugins/
    4. Project plugins in <CWD>/.spruce_grove/plugins/
