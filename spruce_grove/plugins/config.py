...
JSON list in ``grove.cfg`` controls which plugins are suppressed at runtime.
...
from spruce_grove.config import get_value, get_truthy_bool_value, set_value
...
def is_builtin_plugin(plugin_name: str) -> bool:
    ...
    import spruce_grove.plugins as plugins_pkg
...
def get_disabled_plugins() -> Set[str]:
    ...
    Reads from ``disabled_plugins`` config key (JSON list in grove.cfg).
    """
