"""Bounded, recoverable I/O for Spruce Grove's hand-edited JSON config files.

The JSON counterpart to :mod:`spruce_grove.config_file` -- built on the same
:mod:`spruce_grove.atomic_io` primitives -- for state files users are expected
...
Unlike INI config (which is exclusively owned by Spruce Grove and safe to
...
def load_json(path: str, default: Any = None, max_bytes: int | None = None) -> Any:
    ...
    doesn't auto-quarantine the way :mod:`spruce_grove.config_file` does.
    """
...
def mutate_json(
    ...
    sentinel like :func:`spruce_grove.config_file.mutate_config` has.
