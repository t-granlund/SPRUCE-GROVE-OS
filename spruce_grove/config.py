...
from spruce_grove.session_storage import compute_scope_key, save_session
...
def _get_xdg_dir(env_var: str, fallback: str) -> str:
    """
    Get directory for spruce_grove files, defaulting to ~/.spruce_grove.

    XDG paths are only used when the corresponding environment variable
    is explicitly set by the user. Otherwise, we use the legacy ~/.spruce_grove
    ...
    Returns:
        Path to the directory for spruce_grove files
    """
    # Use XDG directory ONLY if environment variable is explicitly set
    xdg_base = os.getenv(env_var)
    if xdg_base:
        return os.path.join(xdg_base, "spruce_grove")

    # Default to legacy ~/.spruce_grove for all file types
    return os.path.join(os.path.expanduser("~"), ".spruce_grove")
...
CONFIG_FILE = os.path.join(CONFIG_DIR, "grove.cfg")
...
def get_mcp_unbound_warning_silenced() -> bool:
    ...
    When True, ``spruce_grove.mcp_.manager._warn_unbound_servers`` skips emitting
...
def get_retry_main_strategy() -> str:
    ...
    Delegates to :func:`spruce_grove.agents.retry_profiles.resolve` so the value
    ...
    """
    try:
        from spruce_grove.agents.retry_profiles import resolve
...
def ensure_config_exists():
    ...
    if missing:
        ...
        for key in missing:
            if key == "grove_name":
                ...
                val = input(
                    "What's your name (so Spruce Grove knows its owner)? "
                ).strip()
            else:
    ...
    # port_base: seed so users discover the knob in their generated grove.cfg
...
def get_value(key: str):
    from spruce_grove.shared_credentials import get, is_credential_key
...
def get_locale() -> str:
    ...
    Precedence when seeding: SPRUCE_GROVE_LOCALE env var > persisted ``locale``
    config key > POSIX locale env vars > default (en-US). See
    ``spruce_grove.i18n.locale.detect_locale``.
    """
    from spruce_grove.i18n import ensure_detected
...
def get_config_keys():
    """
    Returns the list of all config keys currently in grove.cfg,
    ...
    # Opt-in Logfire observability (see spruce_grove/observability.py)
...
def set_config_value(key: str, value: str):
    ...
    from spruce_grove.shared_credentials import is_credential_key, save
...
def _parse_mcp_servers_mapping(raw_text: str) -> dict:
    ...
    :mod:`spruce_grove.mcp_.project_config`.
    """
...
def load_mcp_server_configs(*, raise_on_error: bool = False):
    ...
    1. **User-level** \u2014 ``$XDG_CONFIG_HOME/spruce_grove/mcp_servers.json``
       (global, always trusted).
    2. **Project-level** \u2014 ``<CWD>/.spruce_grove/mcp_servers.json``, but ONLY
       ...
       see :mod:`spruce_grove.mcp_.project_config`.
    ...
    """
    from spruce_grove.messaging.message_queue import emit_error
    ...
    try:
        from spruce_grove.mcp_.project_config import load_project_mcp_server_configs
...
def model_supports_setting(
    ...
    if setting in ("thinking_type", "clear_thinking"):
        from spruce_grove.model_utils import supports_glm_thinking

        if supports_glm_thinking(model_name):
            return True
    if setting == "glm_reasoning_effort":
        from spruce_grove.model_utils import supports_glm_reasoning_effort
...
def _warn_no_model_available() -> None:
    ...
    try:
        ...
        emit_warning(
            "\u26a0\ufe0f  No model is configured! Spruce Grove can't talk to an LLM "
            "until you add one.\n"
...
def get_global_model_name():
    """Return the model name for Spruce Grove to use, or ``None`` if unset.
    ...
    2. Otherwise, look at ``model`` in *grove.cfg*
...
def get_custom_model_settings(model_name: str) -> dict:
    ...
    :func:`spruce_grove.model_factory.make_model_settings`.
...
def get_user_agents_directory() -> str:
    ...
    Returns:
        Path to the user's Spruce Grove agents directory.
    """
...
def get_project_agents_directory() -> Optional[str]:
    ...
    Looks for a .spruce_grove/agents/ directory in the current working directory.
    ...
    """
    project_agents_dir = os.path.join(os.getcwd(), ".spruce_grove", "agents")
    ...
    if not command_history_exists:
        try:
            ...
            old_history_file = os.path.join(
                os.path.expanduser("~"), ".spruce_grove_history.txt"
            )
...
def get_agency_level() -> str:
    """
    Checks grove.cfg for 'agency_level' (case-insensitive in value only).
...
def get_mcp_disabled():
    """
    Checks grove.cfg for 'disable_mcp' (case-insensitive in value only).
    Defaults to False if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
    When enabled, Spruce Grove will skip loading MCP servers entirely.
    """
...
def get_grep_output_verbose():
    """
    Checks grove.cfg for 'grep_output_verbose' (case-insensitive in value only).
...
def get_disable_dangerous_command_guard() -> bool:
    """
    Checks grove.cfg for 'disable_dangerous_command_guard' (case-insensitive in value only).
...
def get_auto_save_session() -> bool:
    """
    Checks grove.cfg for 'auto_save_session' (case-insensitive in value only).
...
def get_current_session_name() -> str:
    ...
    name collisions when multiple Spruce Grove instances start concurrently.
...
def pin_current_session_name(name: str) -> str:
    ...
    """
    from spruce_grove.session_lifecycle import is_valid_session_name
...
def auto_save_session_if_enabled(*, force: bool = False) -> bool:
    ...
    try:
        ...
        stats_suffix = ""
        try:
            from spruce_grove.agents.run_stats import AgentRunStats
        ...
        from spruce_grove.session_lifecycle import fire_post_autosave_callback
...
def record_terminal_session(session_name: str, *, overwrite: bool = True) -> None:
    ...
    reassignment while Spruce Grove is running is rare, but possible after a
...
def get_quick_resume_location(
    ...
    if git_root:
        try:
            from spruce_grove.callbacks import get_git_branch
...
def observe_quick_resume_path(target_path: str, *, path_kind: str = "auto") -> bool:
    ...
    even when Spruce Grove was launched from the parent directory.
    """
...
def get_suppress_thinking_messages() -> bool:
    """
    Checks grove.cfg for 'suppress_thinking_messages' (case-insensitive in value only).
    Defaults to False if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
...
def get_smooth_thinking_stream() -> bool:
    """
    Checks grove.cfg for 'smooth_thinking_stream' (case-insensitive in value only).
    Defaults to True if not set.
...
def get_smooth_response_stream() -> bool:
    """
    Checks grove.cfg for 'smooth_response_stream' (case-insensitive in value only).
    Defaults to True if not set.
    Allowed values for OFF: 0, '0', 'false', 'no', 'off' (all case-insensitive).
...
def get_suppress_informational_messages() -> bool:
    """
    Checks grove.cfg for 'suppress_informational_messages' (case-insensitive in value only).
    Defaults to False if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
...
def get_output_level() -> str:
    ...
    (current behaviour).  The value is read from ``grove.cfg`` with the
    key ``output_level``.
...
def load_api_keys_to_environment():
    """Load all API keys from .env and grove.cfg into environment variables.

    Priority order:
    1. .env file (highest priority) - if present in current directory
    2. grove.cfg - fallback if not in .env
    ...
    # grove.cfg is the user's own (trusted) config, so the Azure endpoint
    ...
    # FIREWORKS_API_KEY for local custom providers) so grove.cfg keys hydrate at
    ...
    try:
        from spruce_grove.provider_credentials import all_api_key_env_vars
    ...
    # unrelated names (base URLs, proxies, SPRUCE_GROVE_* toggles) are ignored so
    ...
    # Step 2: Load from grove.cfg, but only if not already set
    # This ensures .env has priority over grove.cfg
...
def get_default_agent() -> str:
    """
    Get the default agent name from grove.cfg.

    Returns:
        str: The default agent name, or "spruce-grove" if not set.
    """
    return get_value("default_agent") or "spruce-grove"


def set_default_agent(agent_name: str) -> None:
    """
    Set the default agent name in grove.cfg.
...
def resolve_port_base(cli_value=None) -> int:
    ...
    CLI --port-base > SPRUCE_GROVE_PORT_BASE env > grove.cfg[port_base] > default.

    Invalid values at any layer are warned about and skipped, not crashed on.
    """
    candidates = (
        (cli_value, "--port-base"),
        (os.environ.get("SPRUCE_GROVE_PORT_BASE"), "SPRUCE_GROVE_PORT_BASE"),
        (get_value("port_base"), "grove.cfg[port_base]"),
    )
