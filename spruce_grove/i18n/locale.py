...
1. ``SPRUCE_GROVE_LOCALE`` environment variable (explicit override)
2. The persisted ``locale`` config key (``grove.cfg``) — the in-app
...
def detect_locale(
    ...
    1. ``SPRUCE_GROVE_LOCALE`` env var (deliberate, explicit override)
    ...
    Args:
        config_value: The persisted ``locale`` config value, if any. Passed
            ...
            dependency on ``spruce_grove.config`` and trivially testable.
        default: Locale to fall back to when nothing else resolves.
    """
    override = normalize_locale(os.environ.get("SPRUCE_GROVE_LOCALE"))
    if override:
        return override
