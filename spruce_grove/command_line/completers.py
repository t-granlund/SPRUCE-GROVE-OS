...
class SetCompleter(Completer):
    ...
    def get_completions(self, document, complete_event):
        ...
        from spruce_grove.command_line.config_apply import MODEL_SETTINGS_ONLY_KEYS
...
def _read_slash_catalog() -> list[dict[str, str]]:
    ...
    try:
        from spruce_grove import callbacks, plugins
...
def build_completer_stack() -> Completer:
    ...
    from spruce_grove.callbacks import get_completion_providers
    ...
    from spruce_grove.command_line.mcp_completion import MCPCompleter
...
def get_prompt_with_active_model(base: str = ">>> ") -> list:
    """Styled prompt fragments: grove [agent] [model] (cwd) >>>.
    ...
    agent_display = current_agent.display_name if current_agent else "spruce-grove"
    ...
    if cwd.startswith(home):
        ...
        cwd_display = cwd
    return [
        ("class:grove class:tui.header", f"{grove}"),
        ("", " "),
        (
