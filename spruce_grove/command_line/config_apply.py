...
def _restart_notice(label: str) -> str:
    return f"{label} changed. Please restart Spruce Grove for this change to take effect."
...
def invalidate_post_write_caches(key: str) -> None:
    ...
    re-reading grove.cfg + validating registries on every call. After a
    ...
    """
    if key == "yolo_mode":
        from spruce_grove.config import set_cli_yolo_override

        set_cli_yolo_override(None)
    elif key == "model":
        from spruce_grove.config import clear_model_cache, reset_session_model
...
def apply_setting(
    ...
    *,
    ...
    """Persist ``key`` -> ``value`` to ``grove.cfg`` with validation.
    ...
    """
    from spruce_grove.config import set_config_value
    ...
    if key == "cancel_agent_key":
        from spruce_grove.keymap import VALID_CANCEL_KEYS
