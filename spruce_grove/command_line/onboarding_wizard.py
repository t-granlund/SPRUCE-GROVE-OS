...
def should_show_onboarding() -> bool:
    ...
    - SPRUCE_GROVE_SKIP_TUTORIAL env var is set to '1' or 'true'
    """
    # Allow skipping tutorial via environment variable (useful for testing)
    skip_env = os.environ.get("SPRUCE_GROVE_SKIP_TUTORIAL", "").lower()
...
class OnboardingTUI:
    ...
    def _paint(self) -> None:
        ...
        s = menu_style() or RenderStyle.default()
        lines = [
            f"{fg_color(s.bright)}{BOLD_ON}Spruce Grove Tutorial{RESET}",
            "",
...
def require_model_setup_if_needed(wizard_result: Optional[str]) -> None:
    ...
    emit_warning(
        ...
        "   Spruce Grove ships with an empty model list, so you need to add one:\n"
