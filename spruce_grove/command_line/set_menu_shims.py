...
from spruce_grove.config import get_value, get_truthy_bool_value


def get_max_pause_seconds_effective() -> float:
    """Mirrors ``float(get_value('max_pause_seconds') or 180.0)`` at
    ``spruce_grove/agents/event_stream_handler.py``."""
...
def get_disable_mcp_servers_effective() -> bool:
    ...
    There is no typed accessor in :mod:`spruce_grove.config` for this key
