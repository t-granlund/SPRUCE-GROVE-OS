...
:func:`spruce_grove.mcp_.manager._warn_unbound_servers`.

State is persisted in ``grove.cfg`` under ``mcp_unbound_warning_silenced`` so
            ...
            if self.target_silenced:
                emit_info(
                    Text.from_markup(
                        ...
                        "grove.cfg). Run [cyan]/mcp unsilence-warning[/cyan] "
                        "to restore it."
                    ),
                    message_group=group_id,
                )
