"""Command handlers for Spruce Grove - SESSION commands.
...
from spruce_grove.config import AUTOSAVE_DIR
from spruce_grove.i18n import t
from spruce_grove.session_storage import list_sessions, load_session
...
def handle_clear_command(command: str) -> bool:
    ...
    from spruce_grove.agents._builder import reset_model_fallback_warnings
    ...
    from spruce_grove.messaging import emit_info, emit_system_message, emit_warning
...
def handle_compact_command(command: str) -> bool:
    ...
    try:
        from spruce_grove.messaging.run_ui import is_run_active
...
def handle_dump_context_command(command: str) -> bool:
    ...
    from spruce_grove.agents.agent_manager import get_current_agent
    from spruce_grove.messaging import emit_error, emit_warning
...
def handle_load_context_command(command: str) -> bool:
    ...
    from spruce_grove.agents.agent_manager import get_current_agent
    from spruce_grove.config import rotate_session_name
    from spruce_grove.messaging import emit_error, emit_info, emit_success, emit_warning
    from spruce_grove.session_storage import compute_scope_key
