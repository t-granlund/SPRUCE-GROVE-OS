...
Launched from :mod:`spruce_grove.command_line.agent_menu` (and reused by the
post-install flow in :mod:`spruce_grove.command_line.mcp.install_command`).
...
from spruce_grove.mcp_.agent_bindings import (
    get_bound_servers,
    is_bound,
    set_binding,
    toggle_auto_start,
...
from spruce_grove.messaging import emit_info, emit_warning
...
def _agent_label(agent: str, server_name: str) -> str:
    from spruce_grove.mcp_.agent_bindings import get_auto_start
...
async def prompt_bind_after_install(server_name: str) -> None:
    ...
    """
    from spruce_grove.agents import get_available_agents
