"""Command handlers for Spruce Grove - CORE commands.
...
from spruce_grove.command_line.agent_menu import interactive_agent_picker
...
def get_commands_help():
    ...
    from spruce_grove.command_line.command_handler import get_commands_help as _gch
...
def handle_cd_command(command: str) -> bool:
    ...
    if len(tokens) >= 2:
        ...
        if os.path.isdir(target):
            ...
            try:
                from spruce_grove.command_line import file_index
...
def handle_tutorial_command(command: str) -> bool:
    ...
    if result == "chatgpt":
        ...
        from spruce_grove.callbacks import on_claude_oauth_authenticate
...
def handle_agent_command(command: str) -> bool:
    ...
    from spruce_grove.agents import (
...
def handle_add_model_command(command: str) -> bool:
    ...
    from spruce_grove.command_line.add_model_menu import interactive_model_picker
...
def handle_model_settings_command(command: str) -> bool:
    ...
    """
    from spruce_grove.command_line.model_settings_menu import (
...
def handle_mcp_command(command: str) -> bool:
    ...
    from spruce_grove.command_line.mcp import MCPCommandHandler
...
def handle_undo_command(command: str) -> bool:
    ...
    from spruce_grove.messaging import emit_info, emit_error
