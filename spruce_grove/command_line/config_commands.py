"""Command handlers for Spruce Grove - CONFIG commands.
...
from spruce_grove.command_line.config_apply import apply_setting
...
@register_command(
    name="show",
    description="Show grove config key-values",
    usage="/show",
    category="config",
)
def handle_show_command(command: str) -> bool:
    """Show current grove configuration."""
...
@register_command(
    name="set",
    description="Set grove config (e.g., /set yolo_mode true) or launch interactive menu",
    usage="/set [key [value]]",
    category="config",
)
def handle_set_command(command: str) -> bool:
    ...
    from spruce_grove.command_line.set_menu_values import is_sensitive_key, mask_value
...
def _launch_interactive_set_menu() -> None:
    ...
    from spruce_grove.command_line.set_menu import interactive_set_picker
    from spruce_grove.messaging import emit_error, emit_info, emit_success, emit_warning
...
def _get_json_agents_pinned_to_model(model_name: str) -> list:
    ...
    from spruce_grove.agents.json_agent import discover_json_agents
...
def handle_pin_model_command(command: str) -> bool:
    ...
    try:
        if is_json_agent:
            ...
            from spruce_grove.config import set_agent_pinned_model
...
def handle_unpin_command(command: str) -> bool:
    ...
    from spruce_grove.agents.json_agent import discover_json_agents
    from spruce_grove.config import get_agent_pinned_model
    from spruce_grove.messaging import emit_error, emit_info, emit_success, emit_warning
    ...
    try:
        if is_json_agent:
            ...
            from spruce_grove.config import clear_agent_pinned_model
