...
entry point. Everything else in here (grove rules loading, MCP server loading,
...
hook; see :func:`spruce_grove.callbacks.on_wrap_pydantic_agent`.
"""
...
from spruce_grove.agents._compaction import make_history_processor
from spruce_grove.agents._model_message_transform import build_model_message_transform
from spruce_grove.agents._subagent_recursion import build_subagent_recursion_guard
...
_SPRUCE_GROVE_DIR = ".spruce_grove"
...
def load_grove_rules() -> Optional[str]:
    ...
    **Search order for project rules:**
    ...
    1. ``.spruce_grove/AGENTS.md`` (preferred — keeps root clean)
    ...
    # Priority 1: Check .spruce_grove/ directory (preferred location)
    spruce_grove_dir = Path(_SPRUCE_GROVE_DIR)
    if spruce_grove_dir.is_dir():
        for name in _AGENT_RULE_FILES:
            candidate = spruce_grove_dir / name
            if candidate.exists():
                text = _read_rules_text(candidate)
                if text is None:
                    continue
...
def load_mcp_servers(
    ...
    from spruce_grove.tools import tools_disabled
...
def _iter_autostart_targets(manager: Any, agent_name: str):
    ...
    """
    try:
        from spruce_grove.mcp_.agent_bindings import get_bound_servers
...
def _build_gpt_5_6_invoke_agent_guard_text() -> str:
    ...
    """
    # Local import to avoid a top-level ``spruce_grove.config`` cycle -- this
    # module is imported very early during agent construction.
    from spruce_grove.config import get_subagent_recursion_limit_gpt_5_6
...
def _is_gpt_5_6_family(model_name: Optional[str]) -> bool:
    ...
    from spruce_grove.model_utils import supports_gpt_responses_controls
...
def _assemble_instructions(agent: Any, resolved_model_name: str) -> PreparedPrompt:
    """Compose full system prompt + grove rules.
