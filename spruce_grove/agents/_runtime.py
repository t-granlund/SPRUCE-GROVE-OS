...
from spruce_grove.agents import _history, _key_listeners
...
from spruce_grove.agents._non_streaming_render import (
...
from spruce_grove.keymap import sigint_fallback_cancels
...
def _embedded_http_status(text: str) -> Optional[int]:
    ...
    A gateway (e.g. the grove-backend ``custom_anthropic`` proxy) can deliver an
...
def streaming_retry(
    ...
    *policy* -- how many attempts and how long to wait, per role and per model
    -- lives in :mod:`spruce_grove.agents.retry_profiles`; production call sites go
    ...
    **Progress-aware reset.** A retried ``agent.run()`` is a whole-turn re-run,
    but spruce_grove's history processor checkpoints every *completed step* into
    ...
    UI banner, but SRE / power-users grepping ``~/.spruce_grove/logs/errors.log``
            ...
            while True:
                try:
                    ...
                    if delays:
                        ...
                        from spruce_grove.agents.retry_profiles import (
                            MIN_DELAY_SECONDS,
                        )
    ...
    **kwargs: Any,
    ...
    async def _do_run(prompt_to_use: Any) -> Any:
        ...
        async def _observed_event_stream_handler(ctx: Any, events: Any) -> Any:
            from spruce_grove.observability import capture_agent_context
    ...
    async def _run_agent_task_body() -> Any:
        try:
            ...
            from spruce_grove.observability import clear_agent_context
    ...
    try:
        from spruce_grove.mcp_ import manager as _mcp_manager_module
    ...
    from spruce_grove.tools import command_runner as _command_runner
    ...
    def graceful_sigint_handler(_sig, _frame):
        from spruce_grove.keymap import get_cancel_agent_display_name
        from spruce_grove.terminal_utils import reset_windows_terminal_full
