...
class JSONAgent(BaseAgent):
    ...
    def _load_config(self) -> Dict:
        ...
        Bounded via :mod:`spruce_grove.atomic_json` -- a pathologically large
        agent file dropped in ``~/.spruce_grove/agents/`` can no longer
...
def discover_json_agents() -> Dict[str, str]:
    ...
    1. User agents directory (~/.spruce_grove/agents/)
    2. Project agents directory (<CWD>/.spruce_grove/agents/) - if it exists
