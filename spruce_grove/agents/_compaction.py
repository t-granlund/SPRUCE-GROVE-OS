...
Spruce Grove used to carry ~600 lines of hand-rolled compaction: protected-split
...
What remains here is the Spruce Grove-specific glue:
...
from spruce_grove.messaging import emit_error, emit_success, emit_warning
from spruce_grove.messaging.spinner import format_context_info, update_spinner_context
...
def build_compaction_strategy(
    ...
    """Build the ``FallbackCompaction`` chain from Spruce Grove config.
...
def resolve_agent_model(agent: Any) -> Model:
    ...
    Spruce Grove model names only resolve through ``ModelFactory``, so a bare
...
async def compact(
    ...
    *,
    ...
    try:
        from spruce_grove.callbacks import on_pre_compact
