"""Spruce Grove adapters over termflow's steady-rate streaming engine.

The pacing machinery (buffer + adaptive background drain) lives in
``termflow.stream``; this module just wires it to Spruce Grove's Rich
...
class SmoothTermflowWriter(SmoothWriter):
    """ANSI-atomic typewriter wired to Spruce Grove's pause controller."""
...
def make_thinking_smoother(console: Console) -> Optional[ThinkingStreamSmoother]:
    ...
    if not _is_interactive_target(console.file):
        return None
    try:
        from spruce_grove.config import get_smooth_thinking_stream
...
def make_smooth_termflow_writer(target: TextIO) -> Optional[SmoothTermflowWriter]:
    ...
    if not _is_interactive_target(target):
        return None
    try:
        from spruce_grove.config import get_smooth_response_stream
