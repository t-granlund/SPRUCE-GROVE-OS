...
from spruce_grove.agents.json_agent import JSONAgent, discover_json_agents
from spruce_grove.callbacks import on_agent_reload, on_register_agents
from spruce_grove.i18n import t
from spruce_grove.messaging import emit_success, emit_warning
from spruce_grove.tools.common import atomic_write_text
...
def _discover_agents_locked(message_group_id: Optional[str] = None):
    ...
    import spruce_grove.agents as agents_package
    ...
    for _, modname, _ in pkgutil.iter_modules(agents_package.__path__):
        ...
        try:
            # Import the module
            module = importlib.import_module(f"spruce_grove.agents.{modname}")
    ...
    for _, subpkg_name, ispkg in pkgutil.iter_modules(agents_package.__path__):
        ...
        try:
            ...
            subpkg = importlib.import_module(f"spruce_grove.agents.{subpkg_name}")
            ...
            for _, modname, _ in pkgutil.iter_modules(subpkg.__path__):
                ...
                try:
                    ...
                    module = importlib.import_module(
                        f"spruce_grove.agents.{subpkg_name}.{modname}"
                    )
...
def get_current_agent_name() -> str:
    ...
    Returns:
        ...
        Priority: session agent > config default > 'spruce-grove'.
    """
...
def load_agent(agent_name: str) -> BaseAgent:
    ...
    if agent_name not in _AGENT_REGISTRY:
        # Fallback to spruce-grove if agent not found
        if "spruce-grove" in _AGENT_REGISTRY:
