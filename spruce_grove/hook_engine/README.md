...
```python
from spruce_grove.hook_engine import HookEngine, EventData
...
Hooks can be written using **either** the provider's tool name **or** spruce_grove's
internal tool name — the matcher treats them as equivalent.

### Claude Code → spruce_grove

| Claude Code (`matcher`) | spruce_grove internal | Notes |
|-------------------------|---------------------|-------|
