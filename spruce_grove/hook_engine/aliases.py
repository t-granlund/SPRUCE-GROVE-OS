"""
Tool name alias registry — maps each AI provider's tool names to spruce_grove's
...
Each provider block defines a dict[str, str] mapping:
    "<Provider tool name>" -> "<spruce_grove internal tool name>"
...
CLAUDE_CODE_ALIASES: Dict[str, str] = {
    ...
    # NOTE: the tools below have no direct spruce_grove equivalent yet.
...
GEMINI_ALIASES: Dict[str, str] = {
    # Add Gemini → spruce_grove tool mappings here
}
...
SWARM_ALIASES: Dict[str, str] = {
    # Add Swarm → spruce_grove tool mappings here
}
...
def resolve_internal_name(provider_tool_name: str) -> Optional[str]:
    """
    Return the spruce_grove internal tool name for a given provider tool name,
