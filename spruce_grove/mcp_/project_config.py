...
Spruce Grove loads MCP server definitions from the user-level
``$XDG_CONFIG_HOME/spruce_grove/mcp_servers.json``. This module adds an
*optional*, **trust-gated** project-level source at
``<CWD>/.spruce_grove/mcp_servers.json`` so a team can version-control the MCP
...
project can also ship an agent (``.spruce_grove/agents/*.json``) that declares an
...
like project *plugins* (see :mod:`spruce_grove.plugins.trust`), project MCP
configs are **disabled until the user explicitly trusts them** via
``/mcp trust``.

Trust model (identical philosophy to ``spruce_grove/plugins/trust.py``):

* Trust store lives **user-side** at ``~/.spruce_grove/trusted_mcp.json`` — a repo
...
# ``~/.spruce_grove`` to match plugins/trust.py (legacy home dir, not XDG).
TRUST_STORE_FILE = Path.home() / ".spruce_grove" / "trusted_mcp.json"

# Project-local MCP config path, relative to the current working directory.
PROJECT_MCP_RELPATH = Path(".spruce_grove") / "mcp_servers.json"
...
def get_project_mcp_servers_file(project_root: Optional[Path] = None) -> Optional[Path]:
    ...
    Like :func:`spruce_grove.config.get_project_agents_directory`, this never
    creates anything — the team opts in by committing the file.

    Returns ``None`` when the candidate resolves to the *same file* as
    :data:`spruce_grove.config.MCP_SERVERS_FILE`. That is the case whenever
    ``<CWD>/.spruce_grove/mcp_servers.json`` and the user-level config are one
    path — most commonly ``CWD == $HOME`` under the default config location,
    which is the normal state for anyone who just runs ``spruce-grove`` from
...
def _is_user_level_config(candidate: Path) -> bool:
    ...
    """
    from spruce_grove.config import MCP_SERVERS_FILE
...
def load_project_mcp_server_configs(
    ...
    * there's no ``<CWD>/.spruce_grove/mcp_servers.json``, or
    ...
    * the file is malformed.
    ...
    try:
        from spruce_grove.config import _parse_mcp_servers_mapping
...
def _warn_untrusted(project_root: Path, config_file: Path, status: str) -> None:
    ...
    try:
        from spruce_grove.messaging.message_queue import emit_warning
