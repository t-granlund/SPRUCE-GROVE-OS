...
def _with_inherited_ca_bundle(
    ...
    transport does NOT pass spruce_grove's environment to that child
    -- it runs with only the ``env`` dict we hand it (plus a minimal set the
    MCP SDK adds back). So a ``SSL_CERT_FILE`` that lets spruce_grove itself
    ...
    even though spruce_grove, curl, and the browser all work. This copies our
