...
def _raw_tokens_for_message(message: Any) -> int:
    ...
    from spruce_grove.agents._history import stringify_part
...
def _raw_tokens_for_mcp_servers(mcp_servers: Optional[List[Any]]) -> int:
    ...
    from spruce_grove.mcp_.toolset_utils import iter_cached_tool_defs
...
def _kennel_memory_block() -> str:
    ...
    surface this turn.
    """
    from spruce_grove.kennel_provider import get_kennel_recall_block
...
def _agent_tools(agent):
    ...
    try:
        from spruce_grove.agents.base_agent import _extract_pydantic_agent_tools
    except Exception:
        return None
