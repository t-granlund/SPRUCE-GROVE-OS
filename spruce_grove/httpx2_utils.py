...
Spruce Grove's own ``GeminiModel`` -- keep using :mod:`spruce_grove.http_utils`, because those
libraries are still typed against legacy ``httpx``.

Proxy/SSL/HTTP2 resolution is shared with the legacy factory via
:func:`~spruce_grove.http_utils.resolve_proxy_config`, and retry/backoff is shared via
:class:`~spruce_grove.http_retry.RetryingSendMixin`, so behaviour is identical across both
HTTP families.
"""
...
class RetryingAsyncClient(RetryingSendMixin, httpx2.AsyncClient):
    ...
    Same backoff rules as :class:`~spruce_grove.http_utils.RetryingAsyncClient`; only the
...
def create_async_client(
    ...
    Drop-in replacement for :func:`spruce_grove.http_utils.create_async_client`, returning an
