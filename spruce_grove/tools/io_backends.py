"""Pluggable I/O backends for Spruce Grove's tool layer.

By default Spruce Grove's file and shell tools do I/O directly against the local
machine (``open()`` / ``atomic_write_text`` / ``subprocess``). An embedder that
hosts Spruce Grove inside another environment -- an IDE, a sandbox, or an editor
...
* ``FileSystemBackend`` -- **synchronous** (file tools run in Spruce Grove's tool
...
class FileSystemBackend(Protocol):
    ...
    * **Paths** are always absolute and already normalized by the core
      (``spruce_grove.tools.common.resolve_path``) before the backend is called.
...
class ExecResult:
    ...
    interleave them, matching Spruce Grove's own streaming shell behavior).
    """
