...
def should_use_codex_patch(model_name: str | None) -> bool:
    ...
    This deliberately uses Spruce Grove's model aliases rather than a broad
...
NO_TOOLS_ENV_VAR = "SPRUCE_GROVE_NO_TOOLS"


def tools_disabled() -> bool:
    """True when the ``SPRUCE_GROVE_NO_TOOLS`` kill-switch is active.
...
def has_extended_thinking_active(model_name: str | None = None) -> bool:
    ...
    Returns:
        ...
        "enabled" or "adaptive".
    """
    from spruce_grove.config import get_effective_model_settings, get_global_model_name
...
def register_tools_for_agent(
    ...
    Args:
        ...
        agent_name: Optional logical agent name (e.g. ``"spruce-grove"``).
            Passed to the ``register_agent_tools`` callback so plugins can
            advertise tools per-agent if they want.
    """
    from spruce_grove.config import get_universal_constructor_enabled

    if tools_disabled():
        # --no-tools / SPRUCE_GROVE_NO_TOOLS: register nothing at all. This
    ...
    if os.environ.get("SPRUCE_GROVE_DISABLE_ASK_USER_QUESTION", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
