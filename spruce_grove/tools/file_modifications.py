...
from spruce_grove.tools.file_permission_state import (
...
def _is_user_plugin_tree_path(file_path: str) -> bool:
    """True if *file_path* is inside ``~/.spruce_grove/plugins``.
    ...
    """
    from spruce_grove.plugins import USER_PLUGINS_DIR
...
def _refuse_user_plugin_tree(file_path: str) -> Dict[str, Any] | None:
    ...
    return {
        ...
        "path": file_path,
        "message": (
            "Refused: file tools cannot modify ~/.spruce_grove/plugins. "
...
def _delete_snippet_from_file(
    ...
    try:
        ...
        from spruce_grove.config import get_diff_context_lines
...
async def delete_snippet_from_file_async(
    ...
    if refused is not None:
        return refused
    from spruce_grove.callbacks import on_file_permission_async
