"""
HTTP utilities module for spruce-grove.
...
if TYPE_CHECKING:
    import requests
from spruce_grove.config import get_http2
...
def resolve_proxy_config(verify: Union[bool, str, None] = None) -> ProxyConfig:
    ...
    disable_retry = os.environ.get(
        "SPRUCE_GROVE_DISABLE_RETRY_TRANSPORT", ""
    ).lower() in ("1", "true", "yes")
...
class RetryingAsyncClient(RetryingSendMixin, httpx.AsyncClient):
    ...
    Retry behaviour lives in :class:`~spruce_grove.http_retry.RetryingSendMixin` so the
    ``httpx2`` client in :mod:`spruce_grove.httpx2_utils` shares the exact same backoff
