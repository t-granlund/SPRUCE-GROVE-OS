...
from spruce_grove.pydantic_patches import apply_all_patches
...
from spruce_grove import __version__, callbacks, get_core_plugins_version, plugins
from spruce_grove.agents import get_current_agent
from spruce_grove.asyncio_cleanup import install_httpcore2_shutdown_filter
from spruce_grove.i18n import t, use_detected_locale
...
from spruce_grove.keymap import (
    KeymapError,
    validate_cancel_agent_key,
)
from spruce_grove.messaging import emit_info
from spruce_grove.platform_utils import startup_banner_text
from spruce_grove.terminal_utils import (
    print_truecolor_warning,
    reset_unix_terminal,
    reset_windows_terminal_ansi,
    reset_windows_terminal_full,
)
from spruce_grove.version_checker import default_version_mismatch_behavior
...
def _render_turn_exception(exc: Exception) -> None:
    ...
    provider rate limit) are environment hiccups, not Spruce Grove bugs. They get
    ...
    persisted to ``~/.spruce_grove/logs/errors.log`` so SRE / support can still
    see what actually happened upstream. The friendly UI is for the human,
    not for the audit trail.
    """
    from spruce_grove.agents.base_agent import should_retry_streaming_exception
    from spruce_grove.error_logging import log_error
    ...
    from spruce_grove.messaging.queue_console import get_queue_console
...
async def main():
    ...
    parser.add_argument(
        ...
        "-a",
        type=str,
        help="Specify which agent to use (e.g., --agent spruce-grove)",
    )
    parser.add_argument(
        "--model",
        "-m",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Falls back to $SPRUCE_GROVE_PORT_BASE or 'port_base' in grove.cfg (default 8090). "
    ...
    initialize_command_history_file()
    from spruce_grove.messaging import emit_error, emit_system_message
    ...
    from spruce_grove.config import PORT_PROBE_WIDTH, resolve_port_base
    ...
    # SPRUCE_GROVE_ENABLE_LOGFIRE) is set. Must run before agents spin up so
    # pydantic-ai instrumentation catches every run.
    from spruce_grove.observability import configure_logfire
    ...
    # Load API keys from grove.cfg into environment variables
    ...
    if args.model:
        from spruce_grove.config import _validate_model_exists
    ...
    # One-shot sweep of legacy ~/.spruce_grove/contexts/ into autosaves/ (idempotent
    ...
    try:
        from spruce_grove.session_format_migration import sweep_legacy_pickle_sessions
    ...
    if args.resume:
        ...
        from spruce_grove.config import AUTOSAVE_DIR, pin_current_session_name
        ...
        try:
            ...
            if not lazy_created:
                ...
                if not args.prompt and sys.stdout.isatty():
                    try:
                        from spruce_grove.command_line.autosave_menu import (
    ...
    try:
        ...
        if args.prompt:
            ...
            from spruce_grove.config import set_headless_mode
        ...
        try:
            from spruce_grove.messaging.run_ui import stop_persistent_ui
...
def _use_persistent_prompt() -> bool:
    ...
    False (-> classic plain-input path) when:
      * rollback flag: env SPRUCE_GROVE_CLASSIC_PROMPT=1 or config
        ``classic_prompt`` truthy — protects the eyeball-testing period;
      * SPRUCE_GROVE_NO_TUI=1 (tests / pexpect harnesses);
    ...
    if os.environ.get("SPRUCE_GROVE_CLASSIC_PROMPT", "").strip().lower() in truthy:
        return False
    if os.environ.get("SPRUCE_GROVE_NO_TUI", "").strip() == "1":
        return False
    try:
        from spruce_grove.config import get_value
    ...
    try:
        from spruce_grove.terminal_utils import ensure_windows_vt_processing
...
def _persistent_prompt_parts() -> tuple:
    ...
    try:
        ...
        from spruce_grove.messaging.prompt_prefix_style import (
...
def _prompt_echo_text(task: str):
    ...
    from spruce_grove.callbacks import on_prompt_text_color
...
def _interactive_sigint_guard(_sig, _frame):
    ...
    Ctrl+C in Spruce Grove is a *cancel* gesture, never a *quit* gesture
    ...
    try:
        from spruce_grove.terminal_utils import ensure_ctrl_c_disabled
    ...
    try:
        from spruce_grove.messaging.run_ui import (
...
async def interactive_mode(message_renderer, initial_command: str = None) -> None:
    ...
    from spruce_grove.messaging import emit_info, emit_system_message
    ...
    # Tell the user how relentless the grove is configured to be.
    from spruce_grove.config import get_agency_level
    ...
    if initial_command:
        from spruce_grove.agents import get_current_agent
        from spruce_grove.messaging import emit_info, emit_success, emit_system_message
        ...
        try:
            ...
            try:
                from spruce_grove.tools.command_runner import is_awaiting_user_input
            ...
            if response is not None:
                ...
                from spruce_grove.messaging.messages import AgentResponseMessage
    ...
    # stacks to ~/.spruce_grove/stackdumps/ — the conviction kit for wedged
    # cancellations (a frozen REPL can't be introspected any other way).
    try:
        from spruce_grove.diagnostics import install_stack_dump_handler
    ...
    if _use_persistent_prompt():
        try:
            from spruce_grove.messaging.run_ui import start_persistent_ui
    ...
    while True:
        ...
        try:
            if persistent_prompt:
                ...
                # the SPRUCE_GROVE_CLASSIC_PROMPT escape hatch.
                reset_windows_terminal_ansi()
                task = input(">>> ")
            ...
            reset_windows_terminal_full()
            from spruce_grove.callbacks import on_interactive_turn_cancel
            from spruce_grove.messaging import emit_success, emit_warning
            from spruce_grove.messaging.run_ui import DOUBLE_CTRL_C_WINDOW_S
            ...
            from spruce_grove.messaging import emit_success
        ...
        if is_shell_passthrough(task):
            ...
            from spruce_grove.messaging.run_ui import suspended_run_ui
            ...
            if command_result is True:
                ...
                if command_result == "__AUTOSAVE_LOAD__":
                    ...
                    try:
                        ...
                        if os.getenv("SPRUCE_GROVE_NO_TUI") == "1":
                        ...
                        if use_interactive_picker:
                            ...
                            from spruce_grove.session_storage import load_session
        ...
        if task.strip():
            ...
            from spruce_grove.config import auto_save_session_if_enabled
            ...
            from spruce_grove.messaging import emit_system_message
...
async def run_prompt_with_attachments(
    ...
    *,
    ...
    from spruce_grove.messaging import emit_system_message, emit_warning
    ...
    from spruce_grove.agents.event_stream_handler import set_streaming_console
    ...
    from spruce_grove.agents._run_signals import (
        clear_detach_event,
    ...
    if use_run_ui:
        ...
        from spruce_grove.messaging.run_ui import run_ui
...
async def execute_single_prompt(
    ...
    *,
    ...
    from spruce_grove.messaging.messages import AgentResponseMessage
    from spruce_grove.session_lifecycle import persist_named_session
