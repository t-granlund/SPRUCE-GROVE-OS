...
def _get_disabled_plugins() -> Set[str]:
    ...
    try:
        from spruce_grove.plugins.config import get_disabled_plugins
...
def on_load_prompt():
    ...
    every callsite (agent_spruce_grove, agent_planning, agent_tools, ...) free
...
def on_awaiting_user_input(awaiting: bool) -> List[Any]:
    """Fired whenever spruce-grove starts or stops waiting on the human.
...
def on_register_cli_args(parser: Any) -> List[Any]:
    """Let plugins contribute top-level arguments to the ``spruce-grove`` CLI.
...
def on_register_kennel_memory() -> List[Any]:
    ...
    the neutral ``spruce_grove.kennel_provider`` seam instead of importing the
    plugin directly.
    """
...
async def on_agent_run_cancel(group_id: str) -> List[Any]:
    ...
    """
    from spruce_grove.observability import emit_cancellation
