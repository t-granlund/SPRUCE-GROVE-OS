...
grove.cfg first, then ``os.environ``), a masked display value, and saving a new
value so it takes effect immediately (grove.cfg + current-process env).
...
def get_credential_value(env_var: str) -> Optional[str]:
    ...
    grove.cfg (case-insensitive key) first, then ``os.environ``.
    """
...
def save_credential(env_var: str, value: str) -> None:
    """Persist a credential to grove.cfg and apply it to the current process.
...
# Every provider credential spruce_grove manages a key for; mirrors the names in
