...
monkey patches spruce-grove applies to its dependencies.  Each patch is
...
Logging uses the stdlib ``logging`` module, NOT ``spruce_grove.messaging``:
...
from spruce_grove._pydantic_tool_helpers import (
    _block_reason,
...
def _get_spruce_grove_version() -> str:
    """Get the current spruce-grove version."""
    try:
        return importlib.metadata.version("spruce-grove")
    except Exception:
        return "0.0.0-dev"


def patch_user_agent() -> bool:
    """Patch pydantic-ai's User-Agent to use Code-Cedar's version.
    ...
    - 'Code-Cedar/{version}' for all other models
    ...
    try:
        ...
        version = _get_spruce_grove_version()
        ...
        def _get_dynamic_user_agent() -> str:
            ...
            try:
                from spruce_grove.config import get_global_model_name
        ...
        return _patch_failed(
            "patch_user_agent",
            exc,
            "the Code-Cedar User-Agent is DISABLED; requests use pydantic-ai's default.",
        )
