...
its 1.0 release (pydantic-ai >= 2.35 followed). The rest of Spruce Grove
...
def _model_supports_thinking_updates(model_name):
    if not model_name:
        return False
    from spruce_grove.model_utils import should_use_anthropic_thinking_updates
...
class ClaudeCacheAsyncClient(httpx2.AsyncClient):
    ...
    def _check_stored_token_expiry() -> bool:
        ...
        """
        try:
            from spruce_grove.callbacks import on_check_claude_oauth_token_expiry
    ...
    def _refresh_claude_oauth_token(self) -> str | None:
        try:
            from spruce_grove.callbacks import on_refresh_claude_oauth_token
    ...
    async def _refresh_claude_oauth_token_async(self) -> str | None:
        ...
        try:
            from spruce_grove.callbacks import on_refresh_claude_oauth_token_async
