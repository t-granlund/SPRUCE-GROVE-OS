...
``~/.spruce_grove/contexts/`` (populated only by the obscure ``/dump_context``
slash command) and auto-saved sessions lived in ``~/.spruce_grove/autosaves/``
...
def _legacy_contexts_dir() -> pathlib.Path:
    ...
    from spruce_grove.config import CONTEXTS_DIR
...
def _log_orphan_sidecar(old_sidecar: pathlib.Path, error: Exception) -> None:
    ...
    """
    try:
        from spruce_grove.error_logging import log_error_message
