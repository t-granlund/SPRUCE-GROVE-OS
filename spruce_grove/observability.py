...
Spruce Grove ships with zero telemetry. Setting ``enable_logfire`` (via
``/set enable_logfire true`` or the ``SPRUCE_GROVE_ENABLE_LOGFIRE`` env var)
...
def logfire_opted_in() -> bool:
    ...
    The ``SPRUCE_GROVE_ENABLE_LOGFIRE`` env var wins over the persisted
    ``enable_logfire`` config key so headless/CI runs can opt in without
    touching ``grove.cfg``.
    """
    env = os.environ.get("SPRUCE_GROVE_ENABLE_LOGFIRE", "").strip().lower()
    if env in _TRUTHY:
        return True
