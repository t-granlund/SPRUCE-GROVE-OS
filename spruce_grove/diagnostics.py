...
Dumps are written to ``~/.spruce_grove/stackdumps/`` (never the terminal:
...
DUMP_DIR = Path.home() / ".spruce_grove" / "stackdumps"
...
def dump_stacks(file: TextIO, loop: Optional[asyncio.AbstractEventLoop] = None) -> None:
    ...
    """
    print(f"=== spruce-grove stack dump (pid {os.getpid()}) ===", file=file)
...
def write_stack_dump(
    ...
    try:
        ...
        print(f"[spruce-grove] stack dump written to {path}", file=sys.stderr)
        return path
    except Exception:
