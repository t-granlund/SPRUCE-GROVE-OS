"""
Models development API parser for Spruce Grove.

This module provides functionality to parse and work with the models.dev API,
including provider and model information, search capabilities, and conversion to Spruce Grove
...
# Provider type mapping for Spruce Grove configuration
PROVIDER_TYPE_MAP = {
    "anthropic": "anthropic",
    "openai": "openai",
    "google": "gemini",
...
def convert_to_spruce_grove_config(
    model: ModelInfo, provider: ProviderInfo
) -> Dict[str, Any]:
    """
    Convert a model and provider to Spruce Grove configuration format.
    ...
    Returns:
        Dictionary in Spruce Grove configuration format
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Example: Convert to Spruce Grove config
        if providers and registry.get_models():
            provider = providers[0]
            models = registry.get_models(provider.id)
            if models:
                config = convert_to_spruce_grove_config(models[0], provider)
